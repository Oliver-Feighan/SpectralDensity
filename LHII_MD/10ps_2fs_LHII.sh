...
#SBATCH --error=10ps_2fs_LHII.err
...
dataReporter = StateDataReporter(f'{results_dir_path}/{output_name}.txt', int(float(os.environ["REPORTER_DT"])/float(os.environ["INTEGRATOR_DT"])), totalSteps=steps,
...
print()
print(f"Equilibrating: {equilibrationSteps} steps at {dt} time steps")
...
print(f"Production: {steps} steps at {dt} time steps - total of {os.environ['LENGTH']} ps")
print(f" saving step every {os.environ['REPORTER_DT']} fs - total of {1e3 * int(os.environ['LENGTH'])/int(os.environ['REPORTER_DT'])} frames") 
